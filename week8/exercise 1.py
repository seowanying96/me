...
def greet(name="Towering Timmy"): #done
    """Return a greeting.
...
def put_behind_bars(input_string="very naughty boy"): #done
    ...
    InputBehindBars = "|".join(Letter_List)
...
def pet_filter(letter="a"):
    ...
    pets = [
            ...
            "hedgehog","guppy",]
    # fmt: on
    list_length = len(pets)
    listofthepets = []
    for i in range(list_length):
        if pets[i].count(letter) > 0:
            listofthepets.append(pets[i])

    return listofthepets
...
def best_letter_for_pets():
    ...
    for letter in alphabet_list:
        listofthepets = pet_filter(letter)
        TempLength = len(listofthepets)
...
def random_filler_text(number_of_words=200):
    ...
    for i in range(200):
        ...
        random_list = Dict[str(random_key)]
        random_number = random.randint(0,2)
        random_word = random_list[random_number]
